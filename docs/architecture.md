...
`src/core/workspace.ts` validates portable relative paths, sensitive names and link boundaries, caps file sizes, stages complete-file diffs, and verifies all preimages before applying a batch. Manual saves, approved edits and file operations share a filesystem lock. Each file is atomically replaced; a journal precedes multi-file changes. A later transaction can roll back an interrupted batch only while files still match its recorded before/after contents; conflicting external changes preserve the journal for review. This is not a filesystem-level crash-atomic or power-loss durability guarantee. A separate workspace lock serializes engine execution. Approval revisions bind the exact action and saved run configuration and are rechecked inside that lock.

`src/ui/editor/model.ts` owns CodeMirror document state, shared history, per-group view positions, queued saves and recovery snapshots. Views subscribe to that model rather than owning disposable text buffers. Stale view events and superseded file-load selections cannot overwrite current editing state. `IDE.tsx` renders the explorer, tabs, nested groups and contextual assistant; Agent mode preserves its separate composer/review/history surface. Recovery snapshots are bounded and acknowledged visibly; undo history is not serialized across reloads.
